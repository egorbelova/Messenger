...
class GetChats(APIView):
    ...
    def get(self, request):
        ...
        unread_subquery = MessageRecipient.objects.filter(
        ...
        ).values('message__room').annotate(
            count=Count('id')
        ).values('count')[:1]


        chats = Room.objects.filter(users=request.user).annotate(
            last_message_id=Subquery(last_message_subquery),
            unread_count=Subquery(unread_subquery)
        ).order_by('-last_message_id')
        
        last_message_ids = [chat.last_message_id for chat in chats if chat.last_message_id]
        last_messages = Message.objects.filter(
            id__in=last_message_ids
        ).select_related('user').prefetch_related('file')
        ...
        for chat in chats:
            ...
            last_message_serialized = MessageSerializer(last_message, context={'request': request}).data if last_message else None
            chat_data = {
                'id': chat.id,
                'name': chat.name,
                'users': UserSerializer(chat.users.all(), many=True, context={'request': request}).data,
                'room_type': chat.room_type,
                'image': build_url(request, chat.image.url) if chat.image else None,
    ...
    def serialize_message(self, message, request):
        ...
        return {
            ...
            'files': [
                {
                    'id': file.id,
                    'thumbnail_small_url': build_url(request, file.file.url) if file.file else None,
    ...
    def get(self, request, *args, **kwargs):
        ...
        try:
            ...
            messages_qs = room.messages.filter(
                ...
                recipients__user=request.user,
                recipients__is_deleted=False
            ).select_related('user', 'user__profile', 'reply_to').prefetch_related('file')
...
class MessageViewSet(viewsets.ViewSet):
    ...
    def get_queryset(self, room_id):
        ...
        ).select_related('user', 'user__profile').prefetch_related('file')
    ...
    def create(self, request):
        try:
            ...
            if message_text or files:
                ...
                serialized_message = MessageSerializer(new_message, context={'request': request, 'user_id': user.id}).data
...
def get_general_info(request):
    ...
    try:
        ...
        logger.error(f"Error in get_general_info for user {request.user.id}: {str(e)}", exc_info=True)
...
class RoomListView(APIView):
    def get(self, request):
        ...
        ).values('id', 'name', 'last_message_content', 'last_message_date', 'unread_count')

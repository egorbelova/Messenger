...
class ChatConsumer(AsyncWebsocketConsumer):
    async def connect(self):
        try:
            ...
            print(f"User in connect: {self.user} (is_anonymous: {self.user.is_anonymous})")
            ...
            await self.accept()
            logger.info(f"WebSocket connected: {self.channel_name}, User: {self.user.username}, Group: {self.user_group_name}")
    ...
    async def disconnect(self, close_code):
        try:
            ...
            logger.info(f"WebSocket disconnected: {self.channel_name}, User: {getattr(self.user, 'username', 'Anonymous')}, Code: {close_code}")
        except Exception as e:
    ...
    async def receive(self, text_data):
        try:
            ...
            logger.info(f"Received message from {self.user.username}: {message_type}, Room: {room_id}")
    ...
    async def handle_message_reaction(self, data):
        try:
            ...
            if updated_message:
                serialized_message = await self.serialize_message(updated_message, self.user)
    ...
    def save_message(self, room_id, user, message_content):
        try:
            ...
            message = Message.objects.filter(id=message.id).select_related(
                'user', 'user__profile', 'reply_to'
            ).prefetch_related(
                'file',
                Prefetch(
                    'message_reactions',
                    queryset=MessageReaction.objects.select_related('user', 'user__profile')
                )
            ).first()
    ...
    def update_message_reaction(self, message_id, user, reaction_type):
        try:
            ...
            message = Message.objects.filter(id=message_id).select_related(
                'user', 'user__profile', 'reply_to'
            ).prefetch_related(
                'file',
                'message_reactions',
